...
for (i = 1; i < 5; i++) {
    for (y = 0; y < 20; y++) {
        for (x = 0; x < 30; x++) {
            $("#grid").append("<img src='" + (grid.vals[i][y][x] != 0 ? ("images/" + grid.vals[i][y][x] + ".png") : ("")) +"' class='gridLayer" + i + "' id='" + i + "-" + x.toString() + "-" + y.toString() + "' data-x='" + x + "' data-y='" + y +"' style='bottom: " + y*70 + "px; left: " + (420+ x * 70) + "px'>");
        }
    }
}
